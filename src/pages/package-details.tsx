...
const PackageDetails: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100">
      {/* Hero Section */}
      <div className="relative h-64 sm:h-80 md:h-96 w-full bg-cover bg-center" style={{ backgroundImage: `url('${tour.coverImage}')` }}>
        <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-black/40 to-transparent" />
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-white px-4 sm:px-6">
            <span className="bg-[#1C4D9B] px-3 py-1 rounded-full text-xs sm:text-sm mb-3 sm:mb-4 inline-block">{tour.category}</span>
            <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold mb-3 sm:mb-4 drop-shadow-lg leading-tight">{tour.name}</h1>
            <div className="flex items-center justify-center gap-2 sm:gap-4 text-sm sm:text-lg flex-wrap">
              <div className="flex items-center gap-1 sm:gap-2 bg-black/30 px-2 sm:px-4 py-1.5 sm:py-2 rounded-full backdrop-blur-sm">
                <MapPin className="w-4 h-4 sm:w-5 sm:h-5" /><span className="text-xs sm:text-base">{tour.location}</span>
              </div>
              <div className="flex items-center gap-1 sm:gap-2 bg-black/30 px-2 sm:px-4 py-1.5 sm:py-2 rounded-full backdrop-blur-sm">
                <Clock className="w-4 h-4 sm:w-5 sm:h-5" /><span className="text-xs sm:text-base">{tour.duration}</span>
              </div>
              <div className="flex items-center gap-1 sm:gap-2 bg-black/30 px-2 sm:px-4 py-1.5 sm:py-2 rounded-full backdrop-blur-sm">
                <Mountain className="w-4 h-4 sm:w-5 sm:h-5" /><span className="text-xs sm:text-base">{tour.elevation}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Main Content */}
      <div className="px-2 sm:px-6 lg:px-20 py-6 lg:py-10 pb-20 lg:pb-8">
        <div className="flex flex-col lg:flex-row gap-6">
          {/* Mobile Contact Card - Shows first on mobile */}
          <div className="lg:hidden">
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4 sm:p-6">
              <h3 className="font-bold text-lg sm:text-xl text-gray-800 mb-2">Need Help?</h3>
              <p className="text-gray-500 text-sm mb-4">Get in touch with our travel experts</p>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="font-medium text-gray-700 group-hover:text-green-600 text-sm">+977 9851026840</span>
                  </div>
                </a>
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="font-medium text-gray-700 group-hover:text-blue-600 text-sm">info@example.com</span>
                  </div>
                </a>
              </div>
          ...
          <div className="w-full lg:w-[75%] space-y-4 lg:space-y-6">
            ...
            <div className="rounded-lg p-3 sm:p-6">
              <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Trip at a Glance</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4">
                {tripDetails.map((detail, index) => (
                  <div key={index} className="flex items-center gap-2 lg:gap-3 p-2 lg:p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="font-semibold text-xs lg:text-sm truncate">{detail.value}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Overview Section */}
            <section id="overview" className="rounded-lg p-3 sm:p-6 scroll-mt-20">
              <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Overview</h2>
              <div className="prose max-w-none text-gray-600 text-sm lg:text-base" dangerouslySetInnerHTML={{ __html: tour.overview }} />
            </section>

            {/* Trip Highlights Section */}
            {tour.tripHighlight.length > 0 && (
              <section className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Trip Highlights</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {tour.tripHighlight.map((highlight: string, index: number) => (
                    <div key={index} className="flex items-start gap-3">
                      ...
                      <span className="text-gray-600 text-sm lg:text-base">{highlight}</span>
                    </div>
                  ))}
                </div>
              </section>
            ...
            {tour.visitPlaces.length > 0 && (
              <section className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Places You'll Visit</h2>
                <div className="flex flex-wrap gap-2">
                  {tour.visitPlaces.map((place: string, index: number) => (
                    <span key={index} className="bg-[#1C4D9B]/10 text-[#1C4D9B] px-3 lg:px-4 py-1.5 lg:py-2 rounded-full text-xs lg:text-sm font-medium">
                      {place}
                    </span>
                  ))}
                </div>
            ...
            {tour.attraction.length > 0 && (
              <section id="attraction" className="p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Trip Attractions</h2>
                <div className="space-y-4">
                  {tour.attraction.map((item: any) => (
                    <div key={item._id} className="border-l-4 border-[#1C4D9B] pl-3 lg:pl-4">
                      <div className="text-gray-600 text-sm lg:text-base" dangerouslySetInnerHTML={{ __html: item.description }} />
                    </div>
                  ))}
                </div>
              </section>
            ...
            {tour.itinerary.length > 0 && (
              <section id="itinerary" className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Itinerary</h2>
                  ...
                  {tour.itinerary.map((item: any, i: number) => (
                    <div key={item._id} className="border border-gray-200 rounded-lg overflow-hidden">
                      ...
                      >
                        ...
                        <ChevronDown className={`w-5 h-5 flex-shrink-0 transition-transform ${expandedDays.includes(i) ? "rotate-180" : ""}`} />
                      </button>
              ...
              <section id="includes" className="p-3 sm:p-6 scroll-mt-20">
                ...
                <div className="space-y-3">
                  {tour.inclusion.map((item: any) => (
                    <div key={item._id}>
                      {item.description && (
                        <div className="text-sm lg:text-base text-gray-600" dangerouslySetInnerHTML={{ __html: item.description }} />
                      )}
                    </div>
                  ))}
                </div>
            ...
            {tour.exclusion.length > 0 && (
              <section id="excludes" className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B] flex items-center gap-2 lg:gap-3">
            ...
            {tour.gallery.length > 0 && (
              <section id="gallery" className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Photo Gallery</h2>
            ...
            {tour.gearInfo.length > 0 && (
              <section id="gear" className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Gear Information</h2>
                <div className="space-y-4">
            ...
            {(tour.importantNotice.length > 0 || tour.insurance.length > 0 || tour.whyLoveThisTrek.length > 0) && (
              <section id="info" className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Important Information</h2>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="space-y-3">
                      {tour.importantNotice.map((item: any) => (
                        <div key={item._id} className="p-3 lg:p-4 border-l-4 border-yellow-400 rounded-r-lg">
            ...
            {tour.faq.length > 0 && (
              <section id="faqs" className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Frequently Asked Questions</h2>
                <div className="space-y-3">
                  {tour.faq.map((item: any, i: number) => (
                    <div key={item._id} className="border border-gray-200 rounded-lg overflow-hidden">
                      ...
                      >
                        <span className="font-medium text-sm lg:text-base pr-2">{item.title}</span>
                        <ChevronDown className={`w-5 h-5 flex-shrink-0 transition-transform ${expandedDays.includes(100 + i) ? "rotate-180" : ""}`} />
                      </button>
            ...
            {tour.testimonial.length > 0 && (
              <section id="reviews" className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Customer Reviews</h2>
                <div className="space-y-4">
                  {tour.testimonial.map((review: any) => (
                    <div key={review._id} className="p-3 lg:p-4 border border-gray-200 rounded-lg">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          <p className="font-medium text-base">{review.name || 'Anonymous'}</p>
                          <div className="flex">{renderStars(review.rating || 5)}</div>
                        </div>
                      </div>
                      <p className="text-gray-600 text-sm lg:text-base">{review.message || review.description}</p>
                    </div>
                  ))}
                </div>
              </section>
            ...
            {tour.seasonalTrek.length > 0 && (
              <section className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Best Seasons</h2>
                <div className="space-y-4">
                  {tour.seasonalTrek.map((season: any) => (
                    <div key={season._id} className="p-3 lg:p-4 bg-gray-50 rounded-lg">
                      <h3 className="font-semibold mb-2 text-base">{season.title}</h3>
                      <div className="text-gray-600 text-sm lg:text-base" dangerouslySetInnerHTML={{ __html: season.description }} />
                    </div>
                  ))}
                </div>
              </section>
            ...
            {tour.requirements.length > 0 && tour.requirements.some((req: any) => typeof req === 'object' && req.title) && (
              <section className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Requirements</h2>
                <div className="space-y-3">
                  {tour.requirements
                    .filter((req: any) => typeof req === 'object' && req.title)
                    .map((req: any, index: number) => (
                      <div key={req._id || index} className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          {req.description && (
                            <div className="text-sm lg:text-base text-gray-600" dangerouslySetInnerHTML={{ __html: req.description }} />
                          )}
                        </div>
                      </div>
                    ))}
            ...
            {tour.videos.length > 0 && (
              <section className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Videos</h2>
            ...
            {tour.routeMap && (
              <section className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Route Map</h2>
            ...
            {tour.note && (
              <section className="rounded-lg p-3 sm:p-6 scroll-mt-20">
                <h2 className="text-xl lg:text-2xl font-bold mb-4 text-[#1C4D9B]">Important Note</h2>
                <div className="p-3 lg:p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded-r-lg">
                  <div className="text-gray-700 text-sm lg:text-base" dangerouslySetInnerHTML={{ __html: tour.note }} />
                </div>
              </section>
            )}
          </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 z-50 shadow-lg">
        <Link
          to={`/contact-us?package=${encodeURIComponent(tour.name)}`}
          className="block w-full bg-[#1C4D9B] hover:bg-[#153d7a] text-white text-center font-semibold py-3 px-6 rounded-lg transition-colors"
        >
          Inquire Now
        </Link>
      </div>
